...
export async function startConsumer(
  ...
  return channel.consume(QUEUES.MAIN, async (msg) => {
    ...
    const traceShort   = () => '????????'; // placeholder until we parse
    ...
    try {
      ...
      sendToPoison(channel, msg, 'Invalid JSON: could not parse message body');
      channel.ack(msg);
      return;
    }
    ...
    if (!result.success) {
      ...
      sendToPoison(channel, msg, reason);
      channel.ack(msg);
      return;
    }
    ...
    if (shouldFail(event)) {
      // shouldFail guarantees attempt < MAX_ATTEMPTS for this simulation,
      // so we always retry. In a real system you would check attempt here
      // and route to poison once MAX_ATTEMPTS is exceeded.
